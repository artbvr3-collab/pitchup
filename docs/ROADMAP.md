...
| **1** | **Auth slice** | 🟡 In progress (started 2026-05-25) | Prisma + `User` table + Auth.js v5 (Google OAuth) + `/login` (mockup → real screen) + `/welcome` onboarding + `requireAuth()` helper + onboarding middleware. Maps `app/(auth)/login/page.tsx`, `app/(auth)/welcome/page.tsx`, `src/auth/`. **Etap A done:** `prisma/schema.prisma` (User model), `src/shared/db/prisma.ts`, `src/shared/config/env.ts` (Zod), `src/shared/errors/app-error.ts` (AppError + Unauthorized + Validation), `.env.example` updated. Prisma pinned to 6.x — see AGENTS.md gotcha. **Etap B done:** Auth.js v5 config (`src/auth/infrastructure/auth-config.ts`, `auth.ts`, `next-auth.d.ts`), Google provider, JWT 333d, `googleSub` propagated into session, catch-all route handler at `app/api/auth/[...nextauth]/route.ts`. `signIn` ban-check deferred to Etap C (needs `UserRepository`). |
