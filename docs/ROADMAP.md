...
| **1** | **Auth slice** | ▢ Not started | Prisma + `User` table + Auth.js v5 (Google OAuth) + `/login` (mockup → real screen) + `/welcome` onboarding + `requireAuth()` helper + onboarding middleware. Maps `app/(auth)/login/page.tsx`, `app/(auth)/welcome/page.tsx`, `src/auth/`. |
