...
**Status.** Layers 2–8 shipped (Discover + Create + Join/Approve/Reject/Kick/Leave/Cancel/Edit + my-matches + map + cron). Layer 9b added the admin venue write side here (NOT in `moderation`): `covers.ts` (palette + deterministic default), `CreateVenueService` / `UpdateVenueService` (deactivation guard), `VenueRepository` extended with `listAllForAdmin` / `create` / `update` / `countUpcomingNonCancelledAtVenue` — all gated by `requireAdmin()` at the route, no audit row.
